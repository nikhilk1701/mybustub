...
auto LRUKNode::GetSize() -> size_t { return history_.size(); }
  ...
  for (auto &it : node_store_) {
    ...
    if (it.second.IsEvictable()) {
      if (!flg) {
        node = lruk_node;
        flg = true;
      } else if ((node.GetKRecentAccess(k_) != SIZE_MAX && lruk_node.GetKRecentAccess(k_) != SIZE_MAX &&
                  node.GetKRecentAccess(k_) > lruk_node.GetKRecentAccess(k_)) ||
                 (node.GetKRecentAccess(k_) != lruk_node.GetKRecentAccess(k_) &&
                  lruk_node.GetKRecentAccess(k_) == SIZE_MAX)) {
        node = lruk_node;
